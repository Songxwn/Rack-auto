...
| 出网 | 第一次 `bootstrap` 要下载 Ubuntu 26.04.1 live-server ISO（约 **2.7GB**）。默认从 Ubuntu 官方 CD 镜像列表取路径并选延迟最低的（不含阿里云）；也可把 `ubuntu_mirror` 钉死。 |
...
RAMOS 使用 **Ubuntu 26.04.1 LTS live-server**（26.04 首个点发布）。第一次需要联网把 ISO 缓存到本机，抽出 `casper/vmlinuz`、`casper/initrd`，再打一份只含 squashfs 层的 `casper.iso`；之后整个装机网可以离线。PXE 客户端只拉 `casper.iso`，不会把 2.7GB 整包灌进内存。
...
```yaml
bootstrap:
  ubuntu_iso: "/path/to/ubuntu-26.04.1-live-server-amd64.iso"
```
...
1. 勾选「启用内置 DHCP」
2. 「仅响应 PXE 请求」默认关闭（对所有客户端发地址，适合空网段实验室）。若与机房现有 DHCP 同网段、只想管 PXE 引导，再勾选此项
3. **接入网卡**选连着装机交换机的那块（不要选上网网卡）
4. 选中后会按网卡 IPv4 自动填网段、next-server、地址池（一般是 `.100–.200`），可再改
5. 点 **保存并应用**
6. 状态应变为「运行中」，总览页 DHCP 显示 `LIVE`
...
评估版 ISO 可以不填产品密钥。VL / OEM 密钥用「手动填写」。批量环境可选内置 GVLK，并配置可达的 KMS 主机。
